...
describe('ChatInterface Component', () => {
  ...
  test('should send message when Enter key is pressed', async () => {
    ...
    // Verify axios post was called correctly
    expect(mockedAxios.post).toHaveBeenCalledWith('/api/chat/send', { content: 'Create a User entity' });
    expect(mockedAxios.post).toHaveBeenCalledTimes(1);

    // Wait for response
    await waitFor(() => {
      const aiResponse = screen.getByText(/ai: how can i help with your domain model/i);
      expect(aiResponse, 'AI response should be displayed after pressing Enter').toBeInTheDocument();
    });
  });
