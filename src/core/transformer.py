...
class DomainForgeTransformer(Transformer):
    ...
    def start(self, items: List) -> DomainModel:
    ...
    def context_children(self, items: List[Tree]) -> List:
    ...
    def entity_definition(self, items: List[Tree]) -> Entity:
        ...
        if children_node and hasattr(children_node, 'data') and children_node.data == 'entity_children':
            transformed_children = self.entity_children(children_node.children)
    ...
    def entity_children(self, items: List[Tree]) -> List:
    ...
    def value_object_definition(self, items: List[Tree]) -> ValueObject:
        ...
        properties = [item for item in items[1:] if isinstance(item, Property)]
    ...
    def service_definition(self, items: List[Tree]) -> Service:
        ...
        children_node = items[1] if len(items) > 1 else None

        if children_node and hasattr(children_node, 'data') and children_node.data == 'service_children':
            transformed_children = self.service_children(children_node.children)
            for child in transformed_children:
    ...
    def service_children(self, items: List[Tree]) -> List:
    ...
    def repository_definition(self, items: List[Tree]) -> Repository:
        ...
        methods = [item for item in items[1:] if isinstance(item, Method)]
    ...
    def module_definition(self, items: List[Tree]) -> Module:
        ...
        contents = items[1].children if len(items) > 1 and hasattr(items[1], 'children') else []
    ...
    def module_children(self, items: List[Tree]) -> List:
    ...
    def property_definition(self, items: List[Tree]) -> Property:
        ...
        if hasattr(type_def, 'children') and type_def.children:
            simple_type = type_def.children[0]
    ...
    def _extract_type(self, type_node):
        ...
        if hasattr(type_node, 'data'):
            if type_node.data == 'simple_type':
                ...
                container = str(type_node.children[0])
                content = self._extract_type(type_node.children[1])
    ...
    def method_definition(self, items: List[Tree]) -> Method:
        ...
        if index < len(items) and hasattr(items[index], 'data') and items[index].data == 'visibility':
        ...
        parameters = []
        if index < len(items) and hasattr(items[index], 'data') and items[index].data == 'parameter_list':
            param_items = items[index].children
            for param in param_items:
                if hasattr(param, 'data') and param.data == 'parameter':
                    param_name = str(param.children[0])
                    param_type_def = param.children[1]
                    param_type = self._extract_type(param_type_def.children[0])
                    param_default = None
                    if len(param.children) > 2:
                        param_default = param.children[2].children[0] if hasattr(param.children[2], 'children') else param.children[2]
                    parameters.append(Parameter(name=param_name, type=param_type, default_value=param_default))
            index += 1

        # Get return type if present
        return_type = None
        if index < len(items) and hasattr(items[index], 'data') and items[index].data == 'return_type':
            type_def = items[index].children[0]
            return_type = self._extract_type(type_def.children[0])
            index += 1

        # Get description if present
        description = None
        if index < len(items) and hasattr(items[index], 'data') and items[index].data == 'method_body':
            if len(items[index].children) > 0:
                desc_node = items[index].children[0]
                description = str(desc_node.children[0])[1:-1] if hasattr(desc_node, 'children') else str(desc_node)
    ...
    def method_body(self, items: List[Tree]) -> str:
        """Get the method body description if present."""
        if items and hasattr(items[0], 'data') and items[0].data == 'description':
            return str(items[0].children[0])[1:-1] if hasattr(items[0], 'children') else str(items[0])
        return None

    def parameter_list(self, items: List[Tree]) -> List[Parameter]:
    ...
    def parameter(self, items: List[Tree]) -> Parameter:
        ...
        type_def = items[1]
        type_str = str(type_def.children[0]) if hasattr(type_def, 'children') else str(type_def)

        # Handle optional default value
        default_value = None
        if len(items) > 2 and hasattr(items[2], 'data') and items[2].data == 'parameter_default':
    ...
    def relationship_definition(self, items: List[Tree]) -> Relationship:
        ...
        source = items[0].children[0] if hasattr(items[0], 'children') else str(items[0])
        target = items[2].children[0] if hasattr(items[2], 'children') else str(items[2])
        relationship_type = str(items[1])

        description = None
        if len(items) > 3 and hasattr(items[3], 'data') and items[3].data == 'relationship_desc':
            if len(items[3].children) > 0:
                desc = items[3].children[0]
                description = str(desc.children[0])[1:-1] if hasattr(desc, 'children') else str(desc)
    ...
    def relationship_desc(self, items: List[Tree]) -> str:
    ...
    def api_definition(self, items: List[Tree]) -> ApiEndpoint:
        ...
        path = str(items[1])[1:-1]  # Remove quotes from path
        ...
        if index < len(items) and hasattr(items[index], 'data') and items[index].data == 'api_params':
            if len(items[index].children) > 0 and hasattr(items[index].children[0], 'data') and items[index].children[0].data == 'parameter_list':
                parameters = items[index].children[0].children
            index += 1

        # Handle optional return type
        if index < len(items) and hasattr(items[index], 'data') and items[index].data == 'api_return':
            type_def = items[index].children[0]
            return_type = str(type_def.children[0]) if hasattr(type_def, 'children') else str(type_def)
            index += 1

        # Handle optional description
        if index < len(items) and hasattr(items[index], 'data') and items[index].data == 'api_desc':
            if len(items[index].children) > 0:
                desc = items[index].children[0]
                description = str(desc.children[0])[1:-1] if hasattr(desc, 'children') else str(desc)
    ...
    def api_params(self, items: List[Tree]) -> List:
        """Get the API parameters."""
        return items

    def api_return(self, items: List[Tree]) -> str:
        """Get the API return type."""
        return items[0]

    def api_desc(self, items: List[Tree]) -> str:
    ...
    def ui_definition(self, items: List[Tree]) -> UiComponent:
        ...
        # Handle optional parameters
        if index < len(items) and hasattr(items[index], 'data') and items[index].data == 'ui_params':
            if len(items[index].children) > 0 and hasattr(items[index].children[0], 'data') and items[index].children[0].data == 'parameter_list':
                parameters = items[index].children[0].children
            index += 1

        # Handle optional description
        if index < len(items) and hasattr(items[index], 'data') and items[index].data == 'ui_desc':
            if len(items[index].children) > 0:
                desc = items[index].children[0]
                description = str(desc.children[0])[1:-1] if hasattr(desc, 'children') else str(desc)
    ...
    def ui_params(self, items: List[Tree]) -> List:
        """Get the UI parameters."""
        return items

    def ui_desc(self, items: List[Tree]) -> str:
