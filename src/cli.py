...
logger = logging.getLogger("domainforge")


@click.command()
@click.argument("input_file", type=click.Path(exists=True, file_okay=True, dir_okay=False))
@click.option(
    "-o", "--output", "output_dir", type=click.Path(),
    default="./output", help="Output directory for generated code",
)
@click.option(
    "--backend-only", is_flag=True, help="Generate only the backend code",
)
@click.option(
    "--frontend-only", is_flag=True, help="Generate only the frontend code",
)
@click.option(
    "--export-model", is_flag=True, help="Export the domain model as JSON",
)
@click.option(
    "--model-path", type=click.Path(), help="Path where the domain model should be exported",
)
@click.option(
...
def main(
    ...
    try:
        ...
        logger.info(f"Successfully interpreted domain model with {len(domain_model.bounded_contexts)} bounded contexts")

        # Export the domain model if requested
        if export_model:
            export_path = model_path if model_path else str(output_path / "domain_model.json")
        ...
        if not frontend_only:
            ...
            backend_generator = PythonBackendGenerator(output_dir=str(backend_dir))
        ...
        if not backend_only:
            ...
            frontend_generator = TypeScriptFrontendGenerator(output_dir=str(frontend_dir))
